    ...
    public class AuctionResultsController : Controller
    {
        ...
        public IActionResult GetResult(string finde)
        {
            try
            {
                ...
                {
                    var result = _db.AuctionResults.Where(a => a.Number.ToLower().Contains(finde.ToLower())).OrderByDescending(a=> a.Number).ToList();
                    if (result.Count != 0)
                    {
                    ...
                    {
                        result = _db.AuctionResults.Where(a => a.Name.ToLower().Contains(finde.ToLower())).OrderByDescending(a=> a.Number).ToList();
